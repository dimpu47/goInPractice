...
func main() {
	fmt.Println("Walking on a simple integer")
	var one MyInt = 1
	walk(one, 0)

	fmt.Println("Walking on a simple struct")
	two := struct{ Name string }{"foo"}
	walk(two, 0)

	fmt.Println("Walking on a struct with struct fields")
	p := &Person{
		Name:    &Name{"Gaurav Choudhary", "Tyronne", "Ruben"},
		Address: &Address{"Earth", "Asia"},
	}
	walk(p, 0)
}
...
func walk(u interface{}, depth int) {
	...
	if val.Kind() == reflect.Struct {
		for i := 0; i < t.NumField(); i++ {
			...
			fmt.Printf("%sField %q is type %q and kind (%s)\n", tabs, field.Name, field.Type, fieldVal.Kind())
